...
def calculate_demographic_data(print_data=True):
    ...
    num_min_workers = len(min_hour_workers)

    rich_percentage = round(((len(min_hour_workers[min_hour_workers['salary'] == ">50K"]) / num_min_workers) * 100), 1)
    ...
    highest_earning_country = country_percentage.sort_values(ascending=False).index[0]
    ...
    rich_in_india = people_in_india[people_in_india['salary'] == ">50K"]
    rich_occupations_india = rich_in_india.groupby('occupation').size()
    top_IN_occupation = rich_occupations_india.sort_values(ascending=False).index[0]
